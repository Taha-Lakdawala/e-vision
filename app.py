...
# SQL configuration
mysql = MySQL(app)
app.config['MYSQL_HOST'] = 'SG-db1-4995-mysql-master.servers.mongodirector.com'
app.config['MYSQL_USER'] = 'sgroot'
app.config['MYSQL_PASSWORD'] = 'c7Kbt6wGj3Sav+oP'
...
def login():
    ...
    if request.method == "POST":
        ...
        if(usr=="admin"):
            ...
            mycursor = mysql.connection.cursor()
            sql = "SELECT pswd,sname FROM maintable where eno=%s"
            mycursor.execute(sql,(usr,))
            pswd_sname=mycursor.fetchone()
            usr_pswd=pswd_sname[0]
            session['sname']=pswd_sname[1]
            mycursor.close()
            if(psw==usr_pswd):    
                session['username']=usr
                session['Logged_In']=True
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'admin' in session:
            ...
            flash('Unauthorised User',"success")
            return redirect(url_for('index'))
    return decorated_function
...
def NewStudentEntry():
    ...
    if request.method == "POST":
        ...
        flash(f"{enr} has been added","success")
        return redirect("/Home", code=302)
...
def Statistics(): 
    ...
    if totalp_int==0:
        flash("No class has been taken","warning")
        return redirect("/Home", code=302)
    else:
        mycursor =mysql.connection.cursor()
...
def schedulef():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f"Schedule has been updated","success")
            return redirect("/Home", code=302)
...
def clearAtt():
    if request.method == "POST":
        ...
        mysql.connection.commit()
        mycursor.close()
        flash("All attendance data has been removed","success")
        return redirect("/Home", code=302)
...
def updatePswd():
    ...
    if(result==None):
        ...
        mysql.connection.commit()
        mycursor.close()
        flash(f"password of {enr} has been updated","success")
...
def removeStud():
    if request.method == "POST":
        ...
        mysql.connection.commit()
        mycursor.close()
        flash(f"{enr} has been put up on removed list","success")
        return redirect("/Home", code=302)
